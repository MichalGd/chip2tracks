#!/usr/bin/env python3
"""Parse and validate the non-executable chip2tracks configuration format."""
...
BOOLEAN_KEYS = {
    ...
    "MACS3_CALL_SUMMITS", "MACS3_GENERATE_SIGNAL_TRACKS",
    "ALLOW_SINGLE_SAMPLE_CONSENSUS",
    "RUN_IDR",
    "REQUIRE_ALL_ENABLED_TRACKS",
    "GENERATE_CPM_TRACKS", "GENERATE_CPM_PERMISSIVE_TRACKS",
    "GENERATE_CPM_INTERMEDIATE_TRACKS", "GENERATE_CPM_STRINGENT_TRACKS",
    ...
    "ALLOW_FAILED_SPIKEIN",
    "GENERATE_SPIKEIN_CONTROL_TRACKS", "RUN_FRAGMENT_QC",
    "RUN_LIBRARY_COMPLEXITY", "RUN_CROSS_CORRELATION",
    "RUN_REPLICATE_CORRELATION",
    "RUN_TSS_SIGNAL_PROFILE",
...
POSITIVE_INTEGER_KEYS = {
    ...
    "CONSENSUS_MIN_BIOLOGICAL_SAMPLES", "MACS3_SE_EXTSIZE",
    "SE_FRAGMENT_LENGTH_FALLBACK",
...
FLOAT_KEYS = {
    "MACS3_QVALUE", "MACS3_BROAD_CUTOFF", "EPIC2_FDR", "IDR_SOFT_THRESHOLD",
    "IDR_MACS3_PVALUE",
...
ENUMS = {
    ...
    "PRIMARY_PEAK_CALLER": {"auto", "macs3", "epic2"},
    "PEAKCALL_FAILURE_POLICY": {"fail", "continue"},
    "MACS3_KEEP_DUP": {"all", "auto"},
    "MACS3_SE_MODE": {"model", "fixed"},
    "SE_SIGNAL_MODE": {"read", "extend"},
...
def validate(values: dict[str, str], required_keys: set[str]) -> list[str]:
    ...
    se_fragment_length = values.get("SE_FRAGMENT_LENGTH", "")
    if se_fragment_length != "auto":
        try:
            if int(se_fragment_length) <= 0:
                raise ValueError
        except ValueError:
            errors.append("SE_FRAGMENT_LENGTH must be auto or a positive integer")
    ...
    if (values.get("GENERATE_CPM_TRACKS") == "true" and
            values.get("GENERATE_COVERAGE_BIGWIGS") != "true" and
            values.get("GENERATE_COVERAGE_BEDGRAPHS") != "true"):
        errors.append("GENERATE_CPM_TRACKS=true requires at least one coverage output format")

    callers = [item.strip().lower() for item in values.get("PEAK_CALLERS", "").split(",") if item.strip()]
    if not callers or not set(callers) <= {"macs3", "epic2"}:
        errors.append("PEAK_CALLERS must be a comma-separated subset of macs3,epic2")
    ...
    if values.get("RUN_IDR") == "true" and "macs3" not in callers:
        errors.append("RUN_IDR=true requires macs3 in PEAK_CALLERS")
    ...
    for key in ("MACS3_QVALUE", "MACS3_BROAD_CUTOFF", "EPIC2_FDR", "IDR_SOFT_THRESHOLD", "IDR_MACS3_PVALUE",
