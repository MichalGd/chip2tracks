...
condition_levels<-levels(combined$condition);reference<-condition_levels[[1]];numerator<-condition_levels[[2]]
comparison_id<-paste0(numerator,"_vs_",reference,"_target_control_interaction")
comparison_summary<-data.frame(comparison_id=comparison_id,numerator=numerator,reference=reference,
    tested=nrow(result_table),significant=sum(significant),
    higher_in_numerator=sum(significant&result_table$log2FoldChange>0),
    higher_in_reference=sum(significant&result_table$log2FoldChange<0),status="SUCCESS",
    all_results="interaction_results_all.tsv.gz",significant_results="interaction_results_significant.tsv.gz",
    stringsAsFactors=FALSE)
write.table(comparison_summary,file.path(outdir,"comparison_summary.tsv"),sep="\t",quote=FALSE,row.names=FALSE)
