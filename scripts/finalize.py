#!/usr/bin/env python3
"""Write checksums of retained results except the live timing/checksum files."""
...
def main() -> int:
    ...
    files = sorted(path for path in args.output_dir.rglob("*")
                   if path.is_file() and ".checkpoints" not in path.parts
                   and path not in {destination, timing})
    if args.jobs == 1:
