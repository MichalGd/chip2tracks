#!/usr/bin/env python3
"""Validate ChIP-seq/ChIPmentation metadata and emit cohort manifests."""
...
def main() -> int:
    ...
    if "epic2" in args.peak_callers.split(","):
        for row in rows:
            if row["is_control"] == "FALSE" and row["target_class"] == "narrow" and args.primary_peak_caller == "epic2":
                errors.append(
                    f"row {row['_row']}: epic2 cannot be the primary caller for target_class=narrow"
                )
