...
worker() {
    ...
    if is_true "$TRIM_ADAPTERS"; then
        local adapter_args=() extra=() detected="unresolved" evidence="."
        case "$ADAPTER_PRESET" in
            auto) ;;
            illumina) adapter_args+=(--illumina) ;;
            nextera) adapter_args+=(--nextera) ;;
            custom)
                adapter_args+=(--adapter "$CUSTOM_ADAPTER_R1")
                [[ "$layout" == "PE" && -n "$CUSTOM_ADAPTER_R2" ]] && adapter_args+=(--adapter2 "$CUSTOM_ADAPTER_R2")
                ;;
            *) die "unsupported ADAPTER_PRESET: $ADAPTER_PRESET" ;;
        esac
        ...
        if [[ "$layout" == "PE" ]]; then
            run_logged trim_galore --paired --cores "$THREADS_TRIMGALORE" --length "$MIN_TRIMMED_LENGTH" --output_dir "$work" \
                "${adapter_args[@]}" "${extra[@]}" "$raw1" "$raw2" >>"$log" 2>&1
            ...
            run_logged trim_galore --cores "$THREADS_TRIMGALORE" --length "$MIN_TRIMMED_LENGTH" --output_dir "$work" \
                "${adapter_args[@]}" "${extra[@]}" "$raw1" >>"$log" 2>&1
            mv "${work}/${key}.R1.merged_trimmed.fq.gz" "${work}/${key}.R1.trimmed.fastq.gz"
        fi
        if [[ "$ADAPTER_PRESET" == "auto" ]]; then
            evidence="$(grep -Ei 'adapter.*auto|auto.*adapter|adapter type' "$log" | tail -n 1 | tr '\t' ' ' || true)"
            [[ -n "$evidence" ]] && detected="trim_galore_auto" || evidence="no detection line found"
            if [[ "$detected" == "unresolved" ]] && is_true "$ADAPTER_AUTO_DETECTION_REQUIRED"; then
                die "adapter auto-detection was not evidenced for $key; set an explicit preset or inspect $log"
            fi
        else
            detected="$ADAPTER_PRESET"
...
adapter_summary="${OUTPUT_DIR}/01_fastq_qc/adapter_detection.tsv"
printf 'sample_key\tassay_profile\trequested_preset\tresolved_method\tevidence\n' > "$adapter_summary"
for adapter_file in "${OUTPUT_DIR}/01_fastq_qc/adapters/"*.tsv; do
    [[ -e "$adapter_file" ]] || continue
    tail -n 1 "$adapter_file" >> "$adapter_summary"
done
