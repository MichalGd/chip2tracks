...
remove_child() {
    local target="$1" checkpoint_stage="${2:-}" resolved
    ...
    if [[ -n "$checkpoint_stage" ]]; then
        rm -f -- "${OUTPUT_DIR}/.checkpoints/${checkpoint_stage}.json"
        printf '%s\tinvalidated_after_intermediate_cleanup\n' \
            "${OUTPUT_DIR}/.checkpoints/${checkpoint_stage}.json" >> "$manifest"
    fi
}

is_true "$KEEP_TRIMMED_FASTQ" || remove_child "${OUTPUT_DIR}/02_trimmed_fastq" preprocess
is_true "$KEEP_RAW_ALIGNMENT_BAMS" || remove_child "${OUTPUT_DIR}/03_alignment/sorted" alignment
...
if ! is_true "$KEEP_FILTERED_BAMS"; then
    ...
    remove_child "${OUTPUT_DIR}/03_alignment/filtered" filtering
fi
is_true "$KEEP_SPIKEIN_BAMS" || remove_child "${OUTPUT_DIR}/03_alignment/spikein/composite" alignment
is_true "$KEEP_SPIKEIN_BAMS" || remove_child "${OUTPUT_DIR}/03_alignment/spikein/spike" alignment
