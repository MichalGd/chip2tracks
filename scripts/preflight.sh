...
needs_epic2=false
needs_control_fingerprint=false
if [[ ",$PEAK_CALLERS," == *,epic2,* ]] && \
        awk -F '\t' 'NR>1 && $13=="FALSE" && ($9=="broad" || $9=="mixed") {found=1} END{exit !found}' "$SAMPLE_MANIFEST"; then
    needs_epic2=true
fi
if awk -F '\t' 'NR>1 && $13=="FALSE" && $16!="." {found=1} END{exit !found}' "$SAMPLE_MANIFEST"; then
    needs_control_fingerprint=true
fi
...
is_true "$RUN_IDR" && required+=("${IDR_COMMAND}")
is_true "$RUN_PRESEQ" && required+=(preseq)
is_true "$RUN_CROSS_CORRELATION" && required+=("${PHANTOMPEAK_COMMAND}")
is_true "$RUN_CROSS_CORRELATION" && required+=(gzip)
...
is_true "$needs_control_fingerprint" && required+=(plotFingerprint)
...
check_stage_budget() {
    local stage="$1" jobs="$2" threads="$3" requested status
    requested=$((jobs * threads)); status=PASS
    if (( requested > cpu_budget )); then
        status=OVERCOMMITTED
        resource_overcommit=1
...
is_true "$TRIM_ADAPTERS" && version_commands+=(trim_galore cutadapt)
is_true "$needs_epic2" && version_commands+=("$EPIC2_COMMAND")
is_true "$RUN_IDR" && version_commands+=("$IDR_COMMAND")
...
tail -n +2 "$SAMPLE_MANIFEST" | while IFS=$'\t' read -r \
    ...
    [[ "$effective_genome_size" =~ ^[0-9]+$ && "$effective_genome_size" -gt 0 ]] || \
        die "effective genome size must be a positive integer for $genome"
done
...
if [[ "$SPIKEIN_MODE" != "none" ]]; then
    ...
    if [[ -n "$SPIKEIN_BLACKLIST" && "$SPIKEIN_BLACKLIST" != "." ]]; then
        [[ -s "$SPIKEIN_BLACKLIST" ]] || die "SPIKEIN_BLACKLIST is not a readable nonempty file: $SPIKEIN_BLACKLIST"
    fi
fi
