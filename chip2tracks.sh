...
usage() {
    cat <<'USAGE'
Usage: chip2tracks.sh --config config.conf [options]
...
export CHIP2T_CONFIG="$OUTPUT_DIR/00_metadata/resolved_config.conf"
# Backward-compatible internal alias while inherited stage scripts are migrated.
...
if is_true "$PLAN_ONLY"; then
    ...
    printf 'chip2tracks %s plan validated: %s\n' "$VERSION" "$OUTPUT_DIR"
    exit 0
fi
...
run_stage alignment "$SCRIPT_DIR/scripts/align_batch.sh" "$OUTPUT_DIR/03_alignment/sorted" "$OUTPUT_DIR/03_alignment/spikein"
